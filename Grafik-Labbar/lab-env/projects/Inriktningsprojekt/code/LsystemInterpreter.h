...
//Contains segment data, not all fields are currently used. 
struct Segment {
public:
...
class LsystemInterpreter {
	...
	//Generates random nr
	float randomNr(float input, float range);
public: 
	...
	float getTaper() { return taper; }
	float getAngle() { return angleRad; }
	float getAngleRandRange() { return randAngle; }
	float getLengthRandRange() { return randLength; }
	void setDist(float d) { dist = d; }
	void setTaper(float t) { taper = t; }
	//radians
	void setAngle(float a) { angleRad = a; }
	void setAngleRandRange(float a) { randAngle = a; }
	void setLengthRandRange(float l) { randLength = l; }
